...
class KNN_Model(KNN_ConfMtx):
    ...
    def get_neighbor_counts(self, new_image, return_indices=False):
        ...
        distances = np.sqrt(np.sum((self.image_train - new_image)**2,axis=1))
        ...
        nearest_indices =  np.argpartition(distances,self.k_neighbors)[:self.k_neighbors]
        nearest_label = self.label_train[nearest_indices]
        class_counts = np.unique(np.concatenate((self.class_list, nearest_label)), return_counts=True)[1]-1
    ...
    def predict(self, new_image):
        ...
        class_counts = self.get_neighbor_counts(new_image)
        
        class_counts_array = np.argpartition(class_counts,len(class_counts)-1)[-1]
        prediction_label = self.class_list[class_counts_array]
