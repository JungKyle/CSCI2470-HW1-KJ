...
def get_data_MNIST(subset, data_path='../data'):
    ...
    with open(inputs_file_path,'rb') as f, gzip.GzipFile(fileobj=f) as bytestream:
        ...
        image = np.float32(np.frombuffer(bytestream.read(),dtype=np.uint8, count=-1,offset=16))/256

    ## TODO: read the label file
    with open(labels_file_path,'rb') as g, gzip.GzipFile(fileobj=g) as bytestream2:
        
        label = np.frombuffer(bytestream2.read(),dtype=np.uint8, count=-1, offset=8)
...
def get_data_CIFAR(subset, data_path="../data"):
    ...
    for DATA in data_files:
        with open(DATA, 'rb') as fo1:
            BatchFile = pickle.load(fo1, encoding='bytes')
            cifar_dict[b'data'].append(BatchFile[b'data'])
            cifar_dict[b'labels'].append(BatchFile[b'labels'])
    ...
    for k in range(len(label_names)):
        label_names_convert.append(str(label_names[k], 'UTF-8'))

    label_names=label_names_convert    

    label=np.array(np.concatenate(label),dtype=int)
    LabelNew=[]
    for k in range(len(label)):
        LabelNew.append(label_names[label[k]])

    label=np.array(LabelNew)
    ...
    image=np.concatenate(np.array(image),axis=0).reshape(-1,3,32,32).transpose(0,2,3,1)
...
#             label_indice.append(i)
#             ki+=1
#         i+=1
        
        
    
#     image = image_full[label_indice,:,:,:]
#     label = label_full[label_indice]
...
#         image1, label1 = get_specific_class(image_full, label_full, Specific_class, num)
